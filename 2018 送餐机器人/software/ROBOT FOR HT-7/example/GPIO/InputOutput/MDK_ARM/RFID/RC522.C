...
void RC522_Control_IO_Init(void )
{
	
	
	  /* Configure AFIO mode of output pins                                                                     */
  AFIO_GPxConfig(GPIO_PB, AFIO_PIN_6, AFIO_MODE_1); 
  AFIO_GPxConfig(GPIO_PB, AFIO_PIN_7, AFIO_MODE_1); 
	AFIO_GPxConfig(GPIO_PB, AFIO_PIN_8, AFIO_MODE_1);  
	AFIO_GPxConfig(GPIO_PB, AFIO_PIN_9, AFIO_MODE_1); 
  //_GPIO_ClockControl(HT_GPIOB,ENABLE);
  ...
  GPIO_DirectionConfig(HT_GPIOB, GPIO_PIN_7, GPIO_DIR_OUT);
	...
	AFIO_GPxConfig(GPIO_PB, AFIO_PIN_10, AFIO_MODE_1);  //MISO
	GPIO_DirectionConfig(HT_GPIOB, GPIO_PIN_10, GPIO_DIR_IN);  //方向为输入
	GPIO_PullResistorConfig(HT_GPIOB, GPIO_PIN_10, GPIO_PR_UP);  //上拉输入
	GPIO_InputConfig(HT_GPIOB, GPIO_PIN_10, ENABLE);   //使能输入端口
...
//		GPIO_ResetBits(GPIOA, GPIO_Pin_5); //MF522_SCK = 0;
	  MF522_SCK_0;
		MF522_NSS_0;
//    GPIO_ResetBits(GPIOA, GPIO_Pin_4); //MF522_NSS = 0;
        ...
        // MF522_SI = ((ucAddr&0x80)==0x80);
			if(((ucAddr&0x80)==0x80)) 	MF522_MOSI_1;
			else 	MF522_MOSI_0;
//		GPIO_SetBits(GPIOA, GPIO_Pin_5);//MF522_SCK = 1;
			MF522_SCK_1;
			ucAddr <<= 1;
			MF522_SCK_0;
//	GPIO_ResetBits(GPIOA, GPIO_Pin_5);//MF522_SCK = 0;
...
//	GPIO_SetBits(GPIOA, GPIO_Pin_5);//MF522_SCK = 1;
		MF522_SCK_1;
		ucResult <<= 1;
		if(MF522_MISO_0R1) ucResult|=1;
		MF522_SCK_0;
 //     ucResult|=(bit)MF522_SO;
//        GPIO_ResetBits(GPIOA, GPIO_Pin_5);//MF522_SCK = 0;
		}

//		GPIO_SetBits(GPIOA, GPIO_Pin_4);	// MF522_NSS = 1;
		MF522_NSS_1;
		MF522_SCK_1;
//		GPIO_SetBits(GPIOA, GPIO_Pin_5);	//MF522_SCK = 1;
...
unsigned char ReadRawRC(unsigned char Address)
{
     ...
     uint8_t ucResult=0;
	  	SPI_FLASH_CS_LOW();
		ucAddr = ((Address<<1)&0x7E)|0x80;                        // 读数据,地址
...
//    GPIO_ResetBits(GPIOA, GPIO_Pin_5);//MF522_SCK = 0;
    ...
    for(i=8; i>0; i--)
    {
			...
			if(((value&0x80)==0x80)) 	MF522_MOSI_1;
			else 	MF522_MOSI_0;
